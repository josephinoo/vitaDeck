...
fn get_download_class() -> Result<&'static SceDownloadClass> {
    let res = DOWNLOAD_CLASS_SINGLETON.get_or_init(|| unsafe {
        ...
        if load_res < 0 {
            crate::logger::log(&format!("sceKernelLoadStartModule(libshellsvc.suprx) returned {load_res:#010x}"));
        }
...
pub fn start_bgdl(title: &str, url: &str, rif: Option<&[u8]>, _bgdl_type: u32) -> Result<()> {
    crate::logger::log(&format!("[SIMULATED BGDL] Enqueued: '{title}' -> {url} (RIF {} bytes)", rif.map_or(0, |r| r.len())));
    Ok(())
}

#[cfg(target_os = "vita")]
pub fn start_bgdl(title: &str, url: &str, rif: Option<&[u8]>, bgdl_type: u32) -> Result<()> {
    let _ = std::fs::create_dir_all("ux0:bgdl");
